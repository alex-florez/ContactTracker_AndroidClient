...
class RiskContactViewModel @Inject constructor(
   ...
   fun detect() {
        ...
        val user = Location(123.3, 123.4, df.parse("08/06/2021 12:10:00"))
       val positive = Location(1223.3, 1123.4, df.parse("08/06/2021 12:13:00"))

       val result = RiskContactResult()
       val contact = RiskContact()
       val contactLocation = RiskContactLocation(user,positive)
       val withLocation = RiskContactWithLocation(contact, listOf(contactLocation))
       val merge = ResultWithRiskContacts(result, listOf(withLocation))

       viewModelScope.launch {
           riskContactRepository.insert(merge)
           val list = riskContactRepository.getAll()
